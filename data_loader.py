...
def process_bert(data, tokenizer, vocab):
    ...
    for ins_id, instance in tqdm.tqdm(enumerate(data), total=len(data)):
        ...
        # _triu_mask2d = np.triu(np.ones((length, length), dtype=np.bool_), k=1)
        _triu_mask2d = np.ones((length, length), dtype=np.bool_)
        np.fill_diagonal(_triu_mask2d, 0)
        _tri_labels = np.zeros((vocab.tri_label_num, length, length), dtype=np.bool_)
        _arg_labels = np.zeros((vocab.tri_label_num, length, length), dtype=np.bool_)
        _role_labels = np.zeros((vocab.tri_label_num, length, length, vocab.rol_label_num), dtype=np.bool_)
